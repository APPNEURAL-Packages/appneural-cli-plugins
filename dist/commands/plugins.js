...
async function ensurePluginConfigDirectory() {
...
async function ensureCustomPluginDirectory() {
    const directory = getCustomPluginDirectory();
    await fs.mkdir(directory, { recursive: true });
    return directory;
}
async function ensurePluginListed(pkg) {
...
async function removePluginFromConfig(pkg) {
    const config = await readPluginConfig();
...
async function statInstallTime(plugin) {
    try {
...
export function registerPluginsCommands(program) {
    ...
    root
        ...
        statuses.forEach((status) => {
            ...
            logger.info(`${status.candidate.name} v${status.candidate.version ?? "0.0.0"} (${status.candidate.pluginType}) – ${state}`);
        });
    });
    root
        .command("info <name>")
    ...
    root
        ...
        for (const status of statuses) {
            ...
            try {
                ...
                issues += 1;
                logger.warn(`Syntax check failed for ${status.candidate.name}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        if (issues === 0) {
...
function logPluginInfo(pkg) {
    ...
    if (status.loaded) {
        ...
        logger.warn(`${chalk.yellow("⚠")} Plugin ${status.candidate.name} failed to load: ${status.error ?? "unknown error"}`);
    }
}
