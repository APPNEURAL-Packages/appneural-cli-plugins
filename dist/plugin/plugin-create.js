...
export function registerPluginCreateCommand(program) {
    program
        ...
        try {
            ...
            await fs.writeFile(path.join(pluginPath, "package.json"), JSON.stringify(packageJson, null, 2));
            // --------------------------
            // tsconfig.json
            // --------------------------
            const tsconfig = {
            ...
            // src/index.ts
            // --------------------------
            await fs.writeFile(path.join(srcPath, "index.ts"), `import { Command } from "commander";
...
`);
            ...
            // --------------------------
            await fs.writeFile(path.join(srcCommandsPath, "index.ts"), `import { Command } from "commander";
import chalk from "chalk";
...
`);
            ...
            // README.md
            // --------------------------
            await fs.writeFile(path.join(pluginPath, "README.md"), `# @appneural/plugin-${pluginName}
...
`);
            ...
            // .gitignore
            // --------------------------
            await fs.writeFile(path.join(pluginPath, ".gitignore"), `dist
node_modules
*.log
*.tsbuildinfo
`);
            ...
            // .npmignore
            // --------------------------
            await fs.writeFile(path.join(pluginPath, ".npmignore"), `src
tsconfig.json
`);
            // --------------------------
            ...
            await execa("git", ["commit", "-m", `Initial commit for @appneural/plugin-${pluginName}`], { cwd: pluginPath });
