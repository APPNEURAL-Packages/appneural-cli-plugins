...
import { getCustomPluginDirectory, type PluginEnvironment } from "./plugin-context.js";
import { fileExists, buildPluginCandidate, loadRawPackage } from "./plugin-utils.js";
import type { PluginCandidate, PluginSource, PluginRuntimeType } from "./plugin-types.js";
...
export async function discoverPluginCandidates(context: PluginEnvironment): Promise<PluginCandidate[]> {
...
function resolveGlobalModulesPath(): string | null {
  try {
...
async function gatherNodeModulePlugins(
  modulesPath: string,
  source: PluginSource,
  type: PluginRuntimeType,
  results: Map<string, PluginCandidate>
): Promise<void> {
...
async function gatherCustomPlugins(directory: string, results: Map<string, PluginCandidate>): Promise<void> {
...
async function gatherProjectPlugins(context: PluginEnvironment, results: Map<string, PluginCandidate>): Promise<void> {
...
async function gatherConfiguredPlugins(
  context: PluginEnvironment,
  configPath: string,
  source: PluginSource,
  results: Map<string, PluginCandidate>
): Promise<void> {
  ...
  try {
    const data = JSON.parse(await fs.readFile(configPath, "utf-8")) as { plugins?: string[] };
...
async function registerPluginCandidate(
  packageRoot: string,
  source: PluginSource,
...
async function registerLocalPlugin(packageRoot: string, results: Map<string, PluginCandidate>): Promise<void> {
  const candidate = await buildPluginCandidate({
    packageRoot,
    source: "custom",
    pluginType: "local",
...
function isStandalonePlugin(name: string): boolean {
  return name.startsWith("appneural-plugin-");
}

function isScopedPlugin(name: string): boolean {
...
async function resolvePackageFromNodeModules(root: string, packageName: string): Promise<string | null> {
...
async function customPluginExists(nodeModulesRoot: string, pluginName: string): Promise<boolean> {
  if (!nodeModulesRoot) {
    return false;
  }
...
function resolveCustomPluginPath(nodeModulesRoot: string, pluginName: string): string {
  if (!pluginName) {
