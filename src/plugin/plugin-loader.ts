...
let lastLoadedPlugins: PluginCandidate[] = [];
const pluginStatuses = new Map<string, PluginStatus>();
const pluginCommandRegistry = new Map<string, Set<Command>>();

export function getLoadedPlugins(): PluginCandidate[] {
...
export async function reloadPlugins(program: Command, options?: ReloadPluginsOptions): Promise<void> {
...
function dedupeCandidates(candidates: PluginCandidate[]): PluginCandidate[] {
  const deduped = new Map<string, PluginCandidate>();
  for (const candidate of candidates) {
    ...
    if (!existing || getPluginPriority(candidate.pluginType) > getPluginPriority(existing.pluginType)) {
      if (existing) {
        logger.warn(
          `${chalk.yellow("⚠")} Duplicate plugin '${candidate.name}' detected. Preferring ${candidate.pluginType} over ${existing.pluginType}.`
        );
      }
...
export async function loadPlugins(program: Command, options: ReloadPluginsOptions = {}): Promise<void> {
  clearPluginCommands();
  ...
  for (const candidate of candidates) {
    ...
    const status: PluginStatus & { durationMs?: number } = {
      candidate,
      loaded: false
    };
    ...
    try {
      ...
      if (logPluginLifecycle) {
        logger.info(
          `${chalk.green("✔")} Loaded plugin: ${pluginContext.pluginName} (${pluginContext.pluginType}) in ${status.durationMs}ms`
        );
...
function reportFailure(name: string, reason: string): void {
  logger.warn(`${chalk.red("✖")} Plugin failed: ${name} – Reason: ${reason}`);
}

function createPluginCliProxy(cli: Command, pluginName: string): Command {
...
function registerPluginCommand(
  ...
  if (!trimmed) {
    const fallback = parent.command(trimmed, opts as any);
  ...
  const { command: finalCommand, wasCreated } = findOrCreateCommand(
    current,
    finalToken,
    finalDefinition,
    opts,
...
function findOrCreateChildCommand(parent: Command, name: string, pluginName: string): Command {
...
function findOrCreateCommand(
  parent: Command,
  name: string,
  definition: string,
  opts: unknown,
  pluginName: string
): { command: Command; wasCreated: boolean } {
  const existing = parent.commands.find(
    (child) => child.name() === name || child.aliases().includes(name)
  );
  if (existing) {
  ...
  try {
    return { command: parent.command(definition, opts as any), wasCreated: true };
  } catch (error) {
    if (isDuplicateCommandError(error, name)) {
      const conflicting = parent.commands.find(
        (child) => child.name() === name || child.aliases().includes(name)
      );
      if (conflicting) {
...
function splitCommandAndArgs(nameAndArgs: string): { commandTokens: string[]; argsPart: string } {
...
function detachCommand(command: Command): void {
  const parent = command.parent;
  if (!parent) {
    return;
  }
  const parentCommands = parent.commands as Command[];
...
function trackPluginCommand(pluginName: string, command: Command): void {
...
function isDuplicateCommandError(error: unknown, name: string): error is Error {
  return (
...
function createCommandProxy(target: Command, pluginName: string): Command {
  return new Proxy(target, {
    get(commandTarget, prop, receiver) {
      if (prop === "command") {
        return function command(nameAndArgs: string, opts?: unknown) {
          if (typeof nameAndArgs !== "string") {
            const child = commandTarget.command(nameAndArgs, opts as any);
